 ...
 *
 * The clause group is a *lazy* optional (`??`), not a greedy one. Greedy is
 * wrong here: `[\s\S]*?` may cross newlines to reach a `from`, so on
 * `import 'side-effect'` the engine would prefer the clause branch, run past
 * the statement, and capture the *next* statement's specifier instead —
 * silently dropping the side-effect import. Trying the no-clause branch first
 * matches `import 'pkg'` immediately, and still falls back to the clause
 * branch for `import X from 'pkg'`, including multi-line clauses.
 */
const IMPORT_RE =
 ...
 *
 * Exported so `compile.ts` can distinguish "bare npm package, resolvable
 * from the CDN" from "relative/absolute path, genuinely unresolvable in a
 * single-file preview" rather than duplicating the rule.
 */
